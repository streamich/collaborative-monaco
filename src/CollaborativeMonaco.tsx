...
export const CollaborativeMonaco: React.FC<CollaborativeMonacoProps> = ({str, editor: _editor, options, override, onEditor, ...rest}) => {
  const divRef = React.useRef<HTMLDivElement>(null);
  const editorRef = React.useRef<monaco.editor.IStandaloneCodeEditor>(null);

  useIsomorphicLayoutEffect(() => {
    if (!divRef.current) return;
    const editor = _editor ?? monaco.editor.create(divRef.current, options ?? {
      value: str.view(),
      language: 'javascript',
    }, override);
